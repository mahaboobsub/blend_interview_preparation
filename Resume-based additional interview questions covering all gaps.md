...
### Q: Walk me through the architecture and security features of your Password Manager project.

**A:** * **Overview**: I built a secure Password Manager application that allows users to store, retrieve, and generate strong passwords.
* **Architecture**: 
...
### Q: How did you ensure that user passwords were safe from both database leaks and internal server compromises?

**A:** * **Authentication (Protecting the Master Password)**: 
...
### Q: How did you manage user sessions and authentication in your Password Manager?

**A:** * **JWT (JSON Web Tokens)**: Upon successful login (verifying the bcrypt hash of the Master Password), the server issues a JWT.
...
### Q: Why did you choose FastAPI over Node.js for the backend in this project?

**A:** FastAPI was chosen because the core capabilities of our system are Python-heavy:
...
### Q: How did you implement real-time updates for the frontend dashboard?

**A:** We built a centralized ConnectionManager class to manage active WebSocket connections.
...
### Q: Why did you use a dual-database model (PostgreSQL + Neo4j)?

**A:** Each database handles what it does best:
...
### Q: How is database synchronization handled between PostgreSQL and Neo4j?

**A:** To prevent data drift, we mirror primary keys (`donor_id`, `patient_id`) across both databases.
...
### Q: How does the 14-Agent system manage state and recover from failures?

**A:** We define a shared `AgentState` struct utilizing LangGraph's typing. State is passed sequentially through nodes:
...
### Q: How does the hybrid Telegram webhook routing work?

**A:** To prevent LLM hallucinations from corrupting the state of active coordination chains, we built a hybrid handler in the webhook route:
...
### Q: Explain the clinical logic behind the 8-Antigen Matching Engine.

**A:** Thalassemia Major patients receive blood transfusions for life. Over time, their immune systems develop allo-antibodies against minor blood antigens. If they receive blood with these antigens, they can suffer Delayed Hemolytic Transfusion Reactions. We implement an ISBT-weighted compatibility scorer:
...
### Q: Why did you choose Bolna.ai over standard Twilio Voice + gTTS?

**A:** Bolna.ai provides an India-first voice infrastructure:
...
### Q: How did you implement DPDP Act 2023 compliance?

**A:** India's DPDP Act 2023 mandates strict data protection, consent, and user access:
...
### Q: How does the LoRa Offline Bridge function?

**A:** To support remote tribal clinics lacking cellular data, we designed a custom 8-byte packet protocol:
...
### Q: How did you handle the deployment and serving of the heavy CNN model in your Fashion Recommender system?

**A:** * **Model Serving**: Serving a large deep learning model (like VGG16/ResNet) directly within a synchronous web framework can cause massive bottlenecks. Instead, I decoupled the model inference using **FastAPI** (or TensorFlow Serving/ONNX Runtime).
* **Optimization**:
...
### Q: How would you handle a scenario where a user forgets their Master Password in your Zero-Knowledge Password Manager?

**A:** * **The Zero-Knowledge Dilemma**: Because the encryption key is derived entirely from the Master Password on the client side, if the user forgets the Master Password, the server cannot restore access to the encrypted vault. The data is mathematically unrecoverable.
...
### Q: Can you elaborate on the Machine Learning models used in BloodBridge AI for donor churn and patient urgency?

**A:** * **Donor Churn Prediction (XGBoost Classifier)**: 
